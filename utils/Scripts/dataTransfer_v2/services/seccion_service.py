...
from core.interfaces import ISeccionRepository, IDocenteRepository, IAsignaturaRepository, ICursoRepository
...
class SeccionService:
    def __init__(
        self,
        sec_repo: ISeccionRepository,
        doc_repo: IDocenteRepository,
        asig_repo: IAsignaturaRepository,
        cur_repo: ICursoRepository,   # BUG-02 fix: recibido por DI, no instanciado aquí
    ):
    ...
    def importar(self, df: pd.DataFrame, anio: int, periodo: int) -> dict:
        ...
        for _, row in df.iterrows():
            ...
            # Columnas del Excel: apellido y nombre vienen por separado
            ...
            # Buscar docente: 1) por correo institucional, 2) por nombre, 3) por correo genérico
            docente = None
            ...
            if not docente and nombre and apellido:
                nom_norm = normalizar_texto(f"{nombre} {apellido}".strip())
        ...
        if secciones_a_crear:
            ...
            # también las secciones ya existentes (via ON CONFLICT DO UPDATE + RETURNING).
            mapa_secciones = self._sec_repo.insertar_bulk_and_return_ids(secciones_a_crear)
            stats["secciones_nuevas"] = len(mapa_secciones)

            # BUG-02 fix: usar self._cur_repo inyectado por DI.
            # Antes se instanciaba CursoRepository() aquí, rompiendo DI y generando
            # una transacción separada (sin atomicidad con la inserción de secciones).
            cursos_a_insertar = [
                Curso(seccion_id=sec_id, anio=anio, periodo=periodo)
                for sec_id in mapa_secciones.values()
            ]
            stats["cursos_creados"] = self._cur_repo.insertar_bulk(cursos_a_insertar)
