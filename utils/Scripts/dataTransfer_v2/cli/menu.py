...
def seleccion_archivo():
    try:
        ...
        print("⚠️ No se pudo abrir la interfaz gráfica.")
    
    ruta_manual = input("👉 Ingresa la ruta del archivo Excel (o deja vacío para cancelar): ").strip()
    return Path(ruta_manual) if ruta_manual else None

def limpiar_pantalla():
    os.system('cls' if os.name == 'nt' else 'clear')

def seleccionar_hoja(parser: ExcelParser) -> str | None:
    """Muestra las hojas de carreras disponibles y permite elegir una."""
    hojas = parser.listar_hojas_carreras()
    
    if not hojas:
        print("❌ No se encontraron hojas de carreras en el archivo.")
        return None
    
    print("\n📋 Hojas de carreras disponibles:")
