...
export const getReviewsBySection = async (req, res, next) => {
  try {
    const reviews = await sectionService.getReviewsBySection(req.params.id);
    return ApiResponse.success(res, reviews.map(toStudentReviewDto));
  } catch (err) {
    next(err);
  }
};
